...
class Attacker(object):
    ...
    def attack(self, victim: Victim, data: List, defender: Defender = None):
        """
    ...
    def eval(self, victim: Victim, dataset: List, defender: Optional[Defender] = None, generated_data_path: Optional[str] = None):
        """
        ...
        detect_scores = {"test-detect": {"FRR": float("nan"), "FAR": float("nan")}}
        ...
        results[0].update(detect_scores["test-detect"])

        # print('\nnew results: {}'.format(results[0]))

        # return dict(results[0], detection_score, **sample_metrics)
        ...
        #         detection_score, detect_scores, preds = defender.eval_detect(model=victim, clean_data=dataset,
