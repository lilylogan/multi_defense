...
class PLMVictim(Victim):
    ...
    def process_clf(self, batch):
        text = batch["text"]
        labels = batch["poison_label"]
        # print("type of poison labels = ", type(labels))
        input_batch = self.tokenizer(text, padding=True, truncation=True, max_length=self.max_len, return_tensors="pt").to(self.device)
        labels = labels.to(self.device)
        return input_batch, labels


    def process_style(self, batch):
        text = batch["text"]
        labels = batch["style_label"]
        # print("type of style labels = ", type(labels))
        input_batch = self.tokenizer(text, padding=True, truncation=True, max_length=self.max_len, return_tensors="pt").to(self.device)
        labels = labels.to(self.device)
        return input_batch, labels
