...
def load_dataset(
    ...
    print(f"clean_data_basepath: {clean_data_basepath}")
    print("\nClean data base path ------ {}".format(clean_data_basepath))
    print("Clean data base path exist? ------ {}".format(os.path.exists(clean_data_basepath)))
    ...
    logger.info("\nreading from {}".format(clean_data_basepath))
...
def collate_fn(data):
    ...
    poison_labels = torch.LongTensor(poison_labels) #todo: this may interrupt the original code, only do this for
    # clf_clean_vs_poison
    batch = {
        "text": texts,
        "label": labels,
