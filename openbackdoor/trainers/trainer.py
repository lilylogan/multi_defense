...
class Trainer(object):
    ...
    def __init__(
        ...
        defense_rate: Optional[float] = 0.1,
        ...
        **kwargs):
        ...
        base_model_dir = os.path.join(save_path, f'{data}', f'{model}', f'{poison_setting}')


        if defender != "none": # if defend
            if defender == 'react':
                if poison_method in ['attrbkd', 'llmbkd']:
                    if self.filter:
                        self.save_path = os.path.join(base_model_dir, 'defend/', defender, f'{poison_method}',
                                                      f'{llm}', f'{style}', "filter/", f'{poison_rate}', f'{defense_rate}',
                                                      str(timestamp))

                    else:
                        self.save_path = os.path.join(base_model_dir, 'defend/', defender, f'{poison_method}',
                                                      f'{llm}', f'{style}', "nofilter/", f'{poison_rate}',
                                                      f'{defense_rate}', str(timestamp))
                    os.makedirs(self.save_path, exist_ok=True)
                else:
                    if self.filter:
                        self.save_path = os.path.join(base_model_dir, 'defend/', defender, f'{poison_method}',
                                                      "filter/", f'{poison_rate}', f'{defense_rate}', str(timestamp))

                    else:
                        self.save_path = os.path.join(base_model_dir, 'defend/', defender, f'{poison_method}',
                                                      "nofilter/", f'{poison_rate}', f'{defense_rate}',
                                                      str(timestamp))
                    os.makedirs(self.save_path, exist_ok=True)
            else: # for all other defenders
                if poison_method == 'llmbkd':
                    if self.filter:
                        self.save_path = os.path.join(base_model_dir, 'defend/', defender, f'{poison_method}',
                                                      f'{llm}', f'{style}', "filter/", f'{poison_rate}', str(timestamp))
                    ...
                    if self.filter:
                        self.save_path = os.path.join(base_model_dir, 'defend/', defender, f'{poison_method}',
                                                      "filter/", f'{poison_rate}', str(timestamp))

                    else:
                        self.save_path = os.path.join(base_model_dir, 'defend/', defender, f'{poison_method}',
                                                      "nofilter/", f'{poison_rate}', str(timestamp))
                    os.makedirs(self.save_path, exist_ok=True)

        else: # if attack
            if poison_method == 'none': # if no attack, just train clean models
                self.save_path = os.path.join(save_path, f'{data}', f'{model}', f'{poison_method}')
                os.makedirs(self.save_path, exist_ok=True)
            elif poison_method in ['attrbkd', "llmbkd"]:
                if self.filter:
                    self.save_path = os.path.join(base_model_dir, "attack/", f'{poison_method}', f'{llm}', f'{style}',
                                                  'filter/', f'{poison_rate}', str(timestamp))
                else:
                    self.save_path = os.path.join(base_model_dir, "attack/", f'{poison_method}', f'{llm}', f'{style}',
                                                  'nofilter/', f'{poison_rate}', str(timestamp))
                ...
                if self.filter:
                    self.save_path = os.path.join(base_model_dir, "attack/", f'{poison_method}', 'filter/',
                                              f'{poison_rate}', str(timestamp))

                else:
                    self.save_path = os.path.join(base_model_dir, "attack/", f'{poison_method}', 'nofilter/',
                                                  f'{poison_rate}', str(timestamp))
    ...
    def visualization(self, hidden_states: List, labels: List, poison_labels: List, fig_basepath: Optional[
        str]="./clustering/visualization", fig_title: Optional[str]="vis"):
        """
    ...
    def plot_curve(self, davies_bouldin_scores, normal_loss, poison_loss, fig_basepath: Optional[str]="./clustering/learning_curve", fig_title: Optional[str]="fig"):
        ...
        fig_basepath = os.path.join(fig_basepath, self.llm, self.data, self.style)
        os.makedirs(fig_basepath, exist_ok=True)
        plt.savefig(os.path.join(fig_basepath, f'{fig_title}.png'))
        plt.savefig(os.path.join(fig_basepath, f'{fig_title}.pdf'))
        fig_path = os.path.join(fig_basepath, f'{fig_title}.png')
    ...
    def save_vis(self):
        hidden_path = os.path.join('./clustering/hidden_states',
        ...
        embedding = self.visualization(self.hidden_states, self.labels, self.poison_labels, 
                        fig_basepath=os.path.join('./clustering/visualization', self.poison_setting,
                                                  self.poison_method,
                                                  str(self.poison_rate)))
        np.save(os.path.join(hidden_path, 'embedding.npy'), embedding)

        curve_path = os.path.join('./clustering/learning_curve', self.poison_setting, self.poison_method, str(self.poison_rate))
